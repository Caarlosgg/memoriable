...
import { resolverColumnas, columnaDeTarjeta, faseDeColumna } from "../src/lib/boardColumns";
...
describe("resolverColumnas", () => {
  it("sin columnas propias devuelve las tres de siempre, con el id del enum", () => {
    ...
    expect(columnas.map((c) => c.id)).toEqual(["POR_HACER", "EN_PROGRESO", "HECHO"]);
    ...
    expect(columnas.map((c) => c.fase)).toEqual(["POR_HACER", "EN_PROGRESO", "HECHO"]);
  });
  ...
  it("con columnas propias manda su orden, no el del enum", () => {
    ...
    expect(columnas.map((c) => c.nombre)).toEqual(["Pedidos", "En horno", "Hecho"]);
    expect(columnas.every((c) => c.esPersonalizada)).toBe(true);
  });
...
describe("columnaDeTarjeta", () => {
  ...
  it("sin columna propia, cae en la columna por defecto de su fase", () => {
    expect(columnaDeTarjeta({ estado: "EN_PROGRESO", boardStatusId: null }, porDefecto)).toBe("EN_PROGRESO");
  });

  it("con columna propia, manda esa", () => {
    expect(columnaDeTarjeta({ estado: "EN_PROGRESO", boardStatusId: "c3" }, propias)).toBe("c3");
  });
  ...
  it("si su columna ya no existe (borrada), cae en la PRIMERA de su fase — nunca desaparece", () => {
    ...
    expect(columnaDeTarjeta({ estado: "EN_PROGRESO", boardStatusId: "borrada" }, propias)).toBe("c2");
  });

  it("si no queda ninguna columna de su fase, va a la primera que haya en vez de a ninguna", () => {
    const soloHecho = resolverColumnas([{ id: "x", nombre: "Hecho", orden: 0, fase: "HECHO" }]);
    expect(columnaDeTarjeta({ estado: "POR_HACER", boardStatusId: null }, soloHecho)).toBe("x");
  });
});
