...
export function resolverColumnas(
  ...
  if (statuses.length > 0) {
    return [...statuses]
      ...
      .map((s) => ({ id: s.id, nombre: s.nombre, fase: s.fase, esPersonalizada: true }));
  }
...
export function columnaDeTarjeta(
  ...
  if (message.boardStatusId && columnas.some((c) => c.id === message.boardStatusId)) {
...
export function faseDeColumna(columnaId: string, columnas: ColumnaTablero[]): EstadoTarea | null {
